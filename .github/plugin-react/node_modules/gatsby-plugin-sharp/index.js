...
} = require(`./utils`); // const { rgbToHex } = require(`./utils`)
...
function queueImageResizing({
  ...
  const finishedPromise = createJob({
    ...
    outputDir,
    args: {
      isLazy: !(process.env.ENABLE_GATSBY_EXTERNAL_JOBS === `true` || process.env.ENABLE_GATSBY_EXTERNAL_JOBS === `1`) && process.env.gatsby_executing_command === `develop` && !!process.env.GATSBY_EXPERIMENTAL_LAZY_IMAGES,
      operations: [{
