    ...
    visitor: {
      Program(path, state) {
        const nestedJSXVistor = {
          JSXIdentifier(path2) {
            if ((process.env.NODE_ENV === `test` || state.opts.stage === `develop-html` || state.opts.stage === `build-html`) && path2.isJSXIdentifier({
              name: `StaticQuery`
              ...
              const importDeclaration = t.importDeclaration([importDefaultSpecifier], t.stringLiteral(filename ? _path.default.relative(_path.default.parse(filename).dir, resultPath) : shortResultPath));
        ...
        const nestedHookVisitor = {
          CallExpression(path2) {
            if ((process.env.NODE_ENV === `test` || state.opts.stage === `develop-html` || state.opts.stage === `build-html`) && isUseStaticQuery(path2)) {
              const identifier = t.identifier(`staticQueryData`);
              const filename = state.file.opts.filename;
              const staticQueryDir = state.opts.staticQueryDir || `static/d`;
              const shortResultPath = `public/${staticQueryDir}/${this.queryHash}.json`;
        ...
        path.traverse({
          TaggedTemplateExpression(path2) {
            ...
            if (potentialExportPath === null || potentialExportPath === void 0 ? void 0 : potentialExportPath.isExportNamedDeclaration()) {
