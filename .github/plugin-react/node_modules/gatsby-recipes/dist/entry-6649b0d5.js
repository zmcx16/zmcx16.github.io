...
function isDefaultExport(node) {
  if (!node || !t$3.isMemberExpression(node)) {
    return false;
  }
  ...
  if (!t$3.isIdentifier(object) || object.name !== `module`) return false;
  if (!t$3.isIdentifier(property) || property.name !== `exports`) return false;
  return true;
}
...
const isLiteral = node => t$3.isLiteral(node) || t$3.isStringLiteral(node) || t$3.isNumericLiteral(node);

const getValueFromNode = node => {
  if (t$3.isTemplateLiteral(node)) {
...
const getObjectFromNode = nodeValue => {
  ...
  const props = nodeValue.properties.reduce((acc, curr) => {
    ...
    } else if (t$3.isObjectExpression(curr.value)) {
...
const getOptionsForPlugin = node => {
  if (!t$3.isObjectExpression(node) && !t$3.isLogicalExpression(node)) {
    return undefined;
  }
...
const getKeyForPlugin = node => {
  if (t$3.isObjectExpression(node)) {
...
const getNameForPlugin = node => {
  if (t$3.isStringLiteral(node) || t$3.isTemplateLiteral(node)) {
...
const getReadmeForPlugin = async (root, name) => {
  ...
  try {
    if (readmePath) {
      const readme = await fs$1.readFile(readmePath, `utf8`);
      ...
      return {
        visitor: {
          ExpressionStatement(path) {
            ...
            if (shouldAdd) {
              if (t$3.isCallExpression(pluginNodes.value)) {
      ...
      return {
        visitor: {
          ExpressionStatement: path => {
            ...
            if (t$3.isCallExpression(plugins.value)) {
        ...
        visitor: {
          ExpressionStatement(path) {
            ...
            if (siteMetadataExists) {
              ...
              right.properties.unshift(t$3.objectProperty(t$3.identifier(`siteMetadata`), newSiteMetadata));
            }
...
exports.visit = visit;
//# sourceMappingURL=entry-6649b0d5.js.map
