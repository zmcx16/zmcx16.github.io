...
class UrlLoader {
    ...
    async createFormDataFromVariables({ query, variables }) {
        ...
        const { clone, files } = extractFiles.extractFiles(vars, 'variables', ((v) => extractFiles.isExtractableFile(v) || v instanceof Upload || Symbol.asyncIterator in v || isPromise(v)));
    ...
    getExecutorAndSubscriberSync(pointer, options) {
        ...
        let subscriber;
        if (options.enableSubscriptions) {
            const webSocketImpl = this.getWebSocketImpl(options, (moduleName) => require(moduleName));
            subscriber = this.buildSubscriber(pointer, webSocketImpl);
        }
        return {
            executor,
